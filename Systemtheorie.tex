...
\begin{multicols*}{4}
    ...
    \begin{enumerate}
        ...
        \item DGL-System aufstellen \\
              ...
              $\dot\xi = \begin{bmatrix}\lambda_1 & 0 \\ 0 & \lambda_2\end{bmatrix}\xi$ \\
              $\Lambda = \mathbf{Q}^{-1}\mathbf{A}\mathbf{Q} = \begin{bmatrix} \lambda_1 & 0 \\ 0 & \lambda_2 \end{bmatrix}, \xi = \mathbf{Q}^{-1} x', \xi_0=\mathbf{Q}^{-1}x_0'$\\
              $\xi(t)=\begin{bmatrix}e^{\lambda_1(t-t_0)} & 0 \\ 0 & e^{\lambda_2(t-t_0)}\end{bmatrix}\xi_0$
    ...
    $\mathbf{x}\text{reell} = \mathbf{Q}\text{reell}^{-1} \e^{\Delta t}\mathbf{Q}_\text{reell}$\\
    $\dot{\xi}\text{reell} (t) = \Lambda\text{reell}\xi_\text{reell}(t)$\\
    ...
    \textbf{Strudelpunkt}\\
    $\lambda_{1} = \lambda_2^* = \alpha + \beta j$, $\alpha \neq 0$: Je nach $\sgn(\alpha)$ stabiler (-) oder instabiler (+) Strudel in Drehrichtung von $\mathbf{q}r$ ($\xi{\text{reel}1}$) nach $-\mathbf{q}i$ ($\xi{\text{reel}2}$) (für $\mathbf{q}_i > 0$ in Richtung Uhrzeigersinn)\\
    \includegraphics[width=.7\linewidth]{phase-strudel}\\
    \textbf{Wirbelpunkt} (instabil)\\
    $\lambda_{1} = \lambda_2^* = \beta j$: Wirbel in Drehrichtung von $\mathbf{q}r$ ($\xi{\text{reel}1}$) nach $-\mathbf{q}i$ ($\xi{\text{reel}2}$)  (für $\mathbf{q}_i > 0$ in Richtung Uhrzeigersinn)\\
    ...
    $\mathbf{x}{\text{trans}}(t) = \exp{(\mathbf{A}t)} \cdot \mathbf{x}{\text{trans}}(t)$ \\
        ...
        \item Überprüfen des \textbf{Satzes von Hartmann/Grobmann}: \\Für alle Eigenwerte muss gelten: $\Re{\lambda_i} \neq 0$, ansonsten kann man die Linearisierung mit Jacobimatrix nicht anwenden (und muss ggf. mit konservativen Systemen argumentieren)
        \item Phasenportrait zeichnen (lokale Phasenportraits (Phasenportrait zu einem bestimmten GGP) stetig verbinden)
        \end{enumerate}
    ...
    $\dot{E} = 0 \Leftrightarrow \frac{\partial E(\mathbf{x})}{\partial x_1} \dot{x}(t)_1 + \frac{\partial E(\mathbf{x})}{\partial x_2} \dot{x}(t)_2 + \hdots + \frac{\partial E(\mathbf{x})}{\partial x_n} \dot{x}(t)_n = 0$
