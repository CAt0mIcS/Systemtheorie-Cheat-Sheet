...
\begin{multicols*}{4}
    ...
    \sectionbox{
        ...
        \begin{tabular}{lll}
            ...
            \parbox{3cm}{\large \textbf{ESB II}  \\ streng linearer Bereich \\ $u_{\ir d} = 0$ \\ $\abs{ u_{\ir out} } \le U_{\ir sat}$} & \parbox{3cm}{\includegraphics[scale = 0.9]{./img/opamp_esb_2.pdf} } \\[4em]
    ...
    $\mathbf{x}(t) = \mathbf{x}{\text{trans}}(t) + \mathbf{x}{\text{steady}}(t)$
    ...
    $\vec X_{\text{trans}}(p)=(p\ma 1 - \ma A)\underbrace{(\vec x(0) - \vec x'(0))}_{\vec x_{\text{trans}}(0)}$\\
    $\mathbf{x}_{\text{trans}}(t) = \exp{(\mathbf{A}t)} \cdot \mathbf{x}_{\text{trans}}(0)$ \\
    ...
    $F(p) = \mathcal L\{f(t)\} = \int_0^\infty f(t) \e^{-pt}dt$ mit $p$: komplexe Frequenz\\
        Rücktransformation: \\
    ...
    $\vec X(p) = \underbrace{(p\ma 1-\ma A)^{-1}\vec x(0)}_{\vec X_{ZI}(p)} + \underbrace{(p\ma 1-\ma A)^{-1}\vec bV(p)}_{\vec X_{ZS}(p)}$\\
    $\vec x(t) = \vec x(0)\e^{\ma At} + \int_0^t \e^{\ma A(t-t')}v(t')\vec b dt'$
        }

        \subsection{Reduzierte Knotenspannungsanalyse}
        \begin{equation*}
            \underbrace{\ma Y_k}_{\text{Knotenadmittanzmatrix}}\vec U_k(p)=\vec I_q(p)
        \end{equation*}
        ...
        \sectionbox{
        \emphbox{
    $H(j\omega ) = H(p) = \mathcal{L}\{h(t)\} = \fr{\text{Ausgang}}{\text{Eingang}}$
        } \\
    ...
    $\equalhat$ Eigenwerte von $\ma A$
        ...
        \vspace{.5em}
    $Y(p)=H(p)V(p) \rightarrow y(t)=h(t) * v(t)$ \\
    $y_{ZS}(t)=\mathcal L\{Y(p)\}$ \\
