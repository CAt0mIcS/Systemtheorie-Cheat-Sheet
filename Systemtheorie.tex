...
\begin{multicols*}{4}
    ...
    Punkt der Kennlinie im u-i-Diagramm, wo die Zustandsgröße gleich 0 ist ($\vec{\dot x}(t)=0$ bzw. $\dot u_C=0$ bzw. $\dot i_L=0$). Daraus folgt $i_C=u_L=0$
    \begin{tabular*}{\columnwidth}{@{\extracolsep\fill}ll@{}}
        Kapazität & Induktivität\\ \mrule
    ...
    \end{itemize}
    \textbf{III. Ausgangssignal $y$ durch die Zustandsvariablen ausdrücken und in Auskoppelmatrix und Durchgriffsmatrix zusammenfassen}
    \begin{itemize}
        \item[] $y=\ma C \vec x + \ma D + \vec v$
    \end{itemize}
    \textbf{IV. Lösen der Zustandsgleichung}
    \begin{itemize}
        \item Autonome Zustandsgleichung \begin{equation*}
                  \vec x(t)=\underbrace{-\ma A^{-1}\ma B\vec v_0}_{x_\infty}+\exp(\ma A(t-t_0))(x_0+\underbrace{\ma A^{-1}\ma B\vec v_0}_{-x_\infty})
              \end{equation*}
        \item Homogene Zustandsgleichung: $\vec{\dot x}(t)=\ma A \vec x(t)$
              \begin{enumerate}
                  \item Eigenwerte $\lambda_1$ und $\lambda_2$
                        \begin{enumerate}
                            \item[a)] Falls gleich und $A=\text{diag}$: $\vec x(t)=\begin{pmatrix} e^{\lambda(t-t_0)x_{01}} \\ e^{\lambda(t-t_0)x_{02}} \end{pmatrix}$
                            \item[b)] Falls gleich und $A \ne \text{diag}$ (TODO VL 28.4.)
                        \end{enumerate}
                  \item Falls $\lambda_1 \ne \lambda_2$ reel: Eigenvektoren $q_1$ und $q_2$ finden
                  \item $x(t)=\vec q_1e^{\lambda_1(t-t_0)}\xi_{01}+\vec q_2e^{\lambda_2(t-t_0)}\xi_{02}$ mit $\vec \xi_0=\ma Q^{-1}\vec x_0$ und $Q=\begin{pmatrix}\vec q_1 && \vec q_2\end{pmatrix}$
                  \item Falls $\lambda_1 \ne \lambda_2$ komplex: (TODO VL 28.4. + next)
              \end{enumerate}
    \end{itemize}
