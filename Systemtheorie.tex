...
\begin{multicols*}{4}
    ...
    \sectionbox{
        ...
        $u \rightarrow R_d i^d$\qquad $i \rightarrow \frac{1}{R_d} u^d \Phi \rightarrow R_d q^d$\qquad$q \rightarrow \frac{1}{R_d}\Phi^d$
        \\
        Im Schaltbild:\\
    ...
    \sectionbox{
        ...
        \begin{tabular}{lll}
            ...
            \parbox{3cm}{\large \textbf{ESB II}  \\ streng linearer Bereich \\ $u_{\ir d} = 0$ \\ $|u_{\ir out}| \le U_{\ir sat}$}    & \parbox{3cm}{\includegraphics[scale = 0.9]{./img/opamp_esb_2.pdf} } \\[4em]
    ...
    \begin{tabular}{ll}
        ...
        \includegraphics[scale = 0.8]{./img/NIK.pdf}             & \includegraphics[scale = 0.2]{./img/potdiffamp.png}                  \\
    ...
    \sectionbox{
        ...
        $\begin{array}{lcl}
                ...
                U_0 = -\frac{I_0}{G_i} & \Leftrightarrow & I_0=-\frac{U_0}{R_i} \\ \hline
            \end{array}$ \\
    ...
    \begin{tabular}{l|l|l|l}
        ...
        Resistivität  & \includegraphics[height=0.4cm]{./img/Resistivitat.pdf}  & $f_R(u,i)$    & $u = U_0 + R i$       \\
        Kapazität     & \includegraphics[height=0.4cm]{./img/Kapazivitat.pdf}   & $f_C(u,q)$    & $q = Q_0 + C u$       \\
                      &                                                         &               & $i(t)=C\dot u(t)$     \\
                      &                                                         &               & $[C]=\frac{As}{V}=F$  \\
        Induktivität  & \includegraphics[height=0.4cm]{./img/Induktivitat.pdf}  & $f_L(i,\Phi)$ & $\Phi = \Phi_0 + L i$ \\
                      &                                                         &               & $u(t)=L\dot i(t)$     \\
                      &                                                         &               & $[L]=\frac{Wb}{A}=H$  \\
        Memristivität & \includegraphics[height=0.4cm]{./img/Memristivitat.pdf} & $f_M(q,\Phi)$ & $\Phi = \Phi_0 + M q$ \\
    \end{tabular}
    ...
    \sectionbox{
        ...
        \textbf{Leistungsmittelwert}: $P_w = \frac{1}{T} \int_0^T u(t)i(t)dt=\Re{P(j\omega)}$\\
        \textbf{Komplexe Leistung}: $P = \frac{1}{2}UI^* = \frac{1}{2}U_m e^{j\phi_u} I_m e^{-j\phi_i} = U_{eff} I_{eff} e^{j(\phi_u-\phi_i)}=$ \\
        $=\frac{1}{2}\Re{Z(j\omega)}|I|^2+j\frac{1}{2}\Im{Z(j\omega)}|I|^2$\\
        $=\frac{1}{2}\Re{Y(j\omega)}|U|^2-j\frac{1}{2}\Im{Y(j\omega)}|U|^2$\\
    ...
    \sectionbox{
        ...
        Für ein lineares Gleichungssystem $\ma Y  \vec u_k = \vec i_q$ gilt:
        \[u_{k,m}=\frac{(-1)^{i+m}\det \ma Y_{im}}{\det \ma Y}\] \\
        wobei aus $Y_{im}$ die $i$-te Zeile und $m$-te Spalte gestrichen wird und $i$ der Index des einzigen Eintrags in $\vec i_q$ ist.
    }
    ...
    \sectionbox{
        ...
        Punkt der Kennlinie im u-i-Diagramm, wo die Ableitung der Zustandsgröße(n) gleich 0 ist ($\vec{\dot x}(t)=0$ bzw. $\dot u_C=0$ bzw. $\dot i_L=0$). Daraus folgt $i_C=u_L=0$
        \begin{tabular*}{\columnwidth}{@{\extracolsep\fill}ll@{}}
            Kapazität & Induktivität\\ \mrule
    ...
    \includegraphics[width=.46\linewidth]{img/graph-instabil}
    ...
    Sprungantwort: $x_\sigma(t) = (1 - e^{-\fr{t}{\tau}})\sigma(t)$
    ...
    \begin{center}
        \includegraphics[width=.5\linewidth]{img/sprungantwort_impulsantwort}
    \end{center}
    ...
    Antwort auf Erregung: $x(t)=h(t) * v(t)=\int_{t_0}^t h(t-t')v(t')\mathrm dt'$ \\
    \begin{center}
    ...
    \sectionbox{
        ...
        DGL: $\dot p(t)=(\alpha - \beta p(t))p(t) \qquad p(t)$ Population \\
        Lösung: $p(t)=\frac{\alpha}{\beta}\frac{1}{1+(\frac{\alpha}{\beta p_0}-1)e^{-\alpha(t-t_0)}}$ mit \\
        $p(t) \rightarrow p_{\infty,2}=\frac{\alpha}{\beta} \quad (t \rightarrow \infty)$
    }


    \section{Systeme zweiten Grades}
    \sectionbox{
        \subsection{Differentialgleichungssystem aufstellen - \textit{By Inspection}}
        \textbf{I. Gleichungssystem finden} mit z.B. Zustandsgrößen $u_C$ und $i_L$
        \begin{itemize}
            \item[a)] $\dot u_C$ in Abhängigkeit von $u_C$, $i_L$ und evtl. Quellen finden
            \item[b)] $\dot i_L$ in Abhängigkeit von $i_L$, $u_C$ und evtl. Quellen finden
        \end{itemize}
        \textbf{II. Zustandsmatrix A und Einkoppelmatrix B aufstellen}
        \begin{itemize}
            \item[a)] Zustandsgrößenvektor $\vec x(t)=[u_C, i_L]^T$
            \item[b)] Erregungsvektor $\vec v=[u_0, i_0]^T$
            \item[c)] $\vec{\dot x}(t)=\ma A \vec x + \ma B \vec v$
        \end{itemize}
        \textbf{III. Ausgangssignal $y$ (z.B. $u_{out}$) durch die Zustandsvariablen ausdrücken und in Auskoppelmatrix und Durchgriffsmatrix zusammenfassen}
        \begin{itemize}
            \item[] $y=\ma C \vec x + \ma D\vec v$
        \end{itemize}
        \textbf{IV. Lösen der Zustandsgleichung}
    }

    \sectionbox{
        \subsection{Differentialgleichungssystem aufstellen - \textit{Zweitorbeschreibung}}
        ...
        \textbf{III. Differentialgleichungssystem aufstellen} \\
        \textbf{IV. Ausgangsgleichung bestimmen}
    }

    \sectionbox{
        \subsubsection{Matrizengrößen}
        $\ma B \in \mathbb R^{2 \times k}, \ma C \in \mathbb R^{j \times 2}, \ma D \in \mathbb R^{j \times k}$ \\
    ...
    \subsubsection{Automomer Fall - konstante Erregung}
    \begin{align*}
        x_\infty = -\ma A^{-1}\ma B \vec v_0 \\
        \vec x(t)=\vec x_\infty+\exp(\ma A(t-t_0))(\vec x_0-\vec x_\infty)
    \end{align*}

    \subsubsection{Homogener Fall - keine Erregung}
    \sectionbox{
        \textbf{I. Substitution} von autonomen System zu homogenen System
        \begin{itemize}
            \item[] $\vec x' = \vec x - \vec x_\infty \Longleftrightarrow \vec x = \vec x'+\vec x_\infty$
        \end{itemize}

        \textbf{II. Neue Zustandsgleichung aufstellen}
        \begin{itemize}
            \item[] $\vec{\dot x}' = \vec{\dot x} = \ma A \vec x + \ma B \vec v_0 = \ma A \vec x'$
        \end{itemize}

        \textbf{III. Zustandsgleichung lösen}
        \begin{itemize}
            \item Eigenwerte $\lambda_1, \lambda_2$ und Eigenvektoren $ \mathbf{q_1}, \mathbf{q_2}$ von $\ma A$ finden
            \item[a)] $\lambda_1 \ne \lambda_2 \in \mathbb R \rightarrow$ Transformation auf Normalform
            \item[b)] $\lambda_1=\lambda_2 \rightarrow$ Transformation auf Jordan-Normalform
                  \begin{itemize}
                      \item[*] Spezialfall: $A$ Diagonalmatrix: $x(t)=e^{\lambda(t-t_0)}\mathbf{x}_0$
                  \end{itemize}
            \item[c)] $\lambda_1=\lambda_2^* \rightarrow$ Transformation auf reelwertige Normalform
        \end{itemize}
    }
    ...
    \sectionbox{
        ...
        \begin{enumerate}
            \item Eigenwerte $\lambda_1, \lambda_2$ und Eigenvektoren $\mathbf{q}_1, \mathbf{q}_2$ berechnen
            \item DGL-System aufstellen \\
                  $\mathbf{Q} =  \begin{bmatrix} \mathbf{q}_1 & \mathbf{q}_2 \end{bmatrix} \qquad \Lambda = \mathbf{Q}^{-1}\mathbf{A}\mathbf{Q} = \begin{bmatrix} \lambda_1 & 0 \\ 0 & \lambda_2 \end{bmatrix}$ \\
                  $\dot\xi(t) = \begin{bmatrix}\lambda_1 & 0 \\ 0 & \lambda_2\end{bmatrix}\xi(t)$ \\
                  $\xi(t) = \mathbf{Q}^{-1} x'(t) \qquad \xi_0=\mathbf{Q}^{-1}x_0'$\\
                  \textbf{Lösung}: $\xi(t)=\begin{bmatrix}e^{\lambda_1(t-t_0)} & 0 \\ 0 & e^{\lambda_2(t-t_0)}\end{bmatrix}\xi_0$
            \item \textbf{Rücktransformation} \\
                  $x'(t) = \mathbf{Q} \xi(t) = c_1e^{\lambda_1(t-t_0)}\mathbf{q_1} + c_2e^{\lambda_2(t-t_0)}\mathbf{q_2}$
            \item $c_1,c_2$ durch Einsetzen der Anfangswerte $x'(t_0)$ bestimmen, bzw. $c_1=\xi_{01}$ und $c_2=\xi_{02}$
        \end{enumerate}
    }
    ...
    \sectionbox{
        ...
        Lösung der Zustandsgleichung:\\ $\xi'(t) = \begin{bmatrix} e^{\lambda(t-t_0)}\xi_{01}'+(t-t_0)e^{\lambda(t-t_0)}\xi_{02}' \\ e^{\lambda(t-t_0)}\xi_{02}' \end{bmatrix}$\\
        Rücktransformation: $\mathbf{x'}(t) = \mathbf{Q}'\xi'(t) = \mathbf{q}_1'\xi_1'(t) + \mathbf{q}_2'\xi_2'(t)$\\
    }

    \subsubsection{Transformation auf reelwertige Normalform}
    \sectionbox{
        Gegeben: $\lambda_1 = \alpha + j\beta, \beta > 0, \lambda_2=\lambda_1^*, \mathbf{q}_r, \mathbf{q}_i$\\
        $\Lambda_\text{reell} =  \begin{bmatrix} \alpha & - \beta \\ \beta & \alpha \end{bmatrix} \qquad \mathbf{Q}_\text{reell} = \begin{bmatrix} \mathbf{q}_r & - \mathbf{q}_i \end{bmatrix}$\\
        $\mathbf{\xi}(t)_\text{reel}=\mathbf{Q}^{-1}_\text{reel}\mathbf{x'}(t) \qquad \xi_0=\mathbf{Q}^{-1}\mathbf{x}_0=\begin{bmatrix} \xi_{01} \\ \xi_{01}^* \end{bmatrix}$\\
        % \mathbf{x'}(t)_\text{reell} = \mathbf{Q}_\text{reell}^{-1} \e^{\Delta t}\mathbf{Q}_\text{reell} \qquad 
        $\dot{\xi}_\text{reell} (t) = \Lambda_\text{reell}\xi_\text{reell}(t) \qquad \xi(t) = \begin{bmatrix} e^{\lambda(t-t_0)}\xi_{01} \\ e^{\lambda^*(t-t_0)}\xi_{01}^* \end{bmatrix}$\\
        $\xi_\text{reel}(t)=\begin{bmatrix} 2\Re{\xi_1(t)} \\ 2 \Im{\xi_1(t)} \end{bmatrix}$ \\
        Rücktransformation: $\mathbf{x'}(t) = c_1e^{\alpha(t-t_0)}[\cos (\beta(t-t_0))\mathbf{q}_r - \sin (\beta(t - t_0))\mathbf{q}_i] + c_2e^{\alpha(t-t_0)}[\sin (\beta(t-t_0))\mathbf{q}_r + \cos (\beta(t - t_0))\mathbf{q}_i]$
    }


    \subsection{Phasenportraits}
    Trajektorien, welche abhängig vom Anfangspunkt sind und sich \textbf{nie} schneiden.

    \subsubsection{Zeichnen des Phasenportraits}
    \sectionbox{
        \textbf{I. Bestimmung der Eigenwerte $\lambda_i$ und Eigenvektoren $q_i$ von $\ma A$}\\
        $\lambda_{1,2} = \frac{tr(\ma A)}{2} \pm \sqrt{\frac{tr(\ma A)^2}{4}-det(\ma A)} \qquad [\lambda] = \frac1s$\\
        $a_{12} \neq 0 \Rightarrow \mathbf{q}i = \begin{bmatrix}-a{12}\\a_{11}-\lambda_i\end{bmatrix}$\quad $a_{21} \neq 0 \Rightarrow \mathbf{q}i = \begin{bmatrix}a{22}-\lambda_i\\-a_{21}\end{bmatrix}$\quad$a_{12} = a_{21} = 0 \Rightarrow \mathbf{q}_1 = \begin{bmatrix}1\\0\end{bmatrix}, \mathbf{q}_2 = \begin{bmatrix}0\\1\end{bmatrix}$\\

        \emphbox{Falls Eigenvektoren komplex: $\mathbf{q}_r = \Re{q_1}$\qquad$\mathbf{q}_i = \Im{q_1}$ mit $\lambda_1$ der EW mit positivem Imaginärteil}
    }

    \textbf{II. Bestimmung des Fixpunktes}\\
    $\mathbf{A}\mathbf{x}\infty + \mathbf{B}\mathbf{v} = 0 \Rightarrow \mathbf{x}\infty = -\mathbf{A}^{-1} \mathbf{B}\mathbf{v}$\\
    \textbf{III. Art des Phasenportraits}\\
    \textbf{Strudelpunkt}\\
    $\lambda_{1} = \lambda_2^* = \alpha + \beta j$, $\alpha \neq 0$: Je nach $\sgn(\alpha)$ stabiler (-) oder instabiler (+) Strudel in Drehrichtung von $\mathbf{q}r$ ($\xi{\text{reel}1}$) nach $-\mathbf{q}i$ ($\xi{\text{reel}2}$) (jeweils Startpunkt zu Endpunkt). Bei stabilem Struden von außen nach innen, bein instabilem Struden andersrum \\
    \includegraphics[width=.7\linewidth]{phase-strudel}\\
    \textbf{Wirbelpunkt} (instabil)\\
    $\lambda_{1} = \lambda_2^* = \beta j$: Wirbel in Drehrichtung von $\mathbf{q}r$ ($\xi{\text{reel}1}$) nach $-\mathbf{q}i$ ($\xi{\text{reel}2}$) \\
    \includegraphics[width=.7\linewidth]{phase-wirbel}\\
    \textbf{Knotenpunkt}\\
    $\lambda_{1,2} \in \mathbb R < 0, |\lambda_1| < |\lambda_2|$: Trajektorien von Richtung (Eigenvektor) des schnelleren Eigenwerts ($\mathbf{q}_2, \xi_2$) schmiegen sich an an Richtung des langsameren Eigenwerts ($\mathbf{q}_1, \xi_1$). Stabil für $\lambda_1,\lambda_2 < 0$, instabil sonst. Falls $\lambda_1=\lambda_2$, so sind Trajektorien Geraden \\
    \includegraphics[width=.6\linewidth]{phase-knoten}\\ \\
    \textbf{Sattelpunkt} (instabil)\\
    ...
    \includegraphics[width=.6\linewidth]{phase-sattel}\\

    \subsubsection{Übersicht Phasenportraits}
    \includegraphics[width=\linewidth]{img/phasenportraits.pdf}


    \textbf{IV. Einzeichnen von GGP und Eigenvektoren}\\
    Die Eigenvektoren werden ausgehend vom GGP eingezeichnet. Bei konjugierten Eigenvektoren zeichnet man den Realteil und den negierten Imaginärteil.\\

    \subsection{Zeitverlauf der Zustandsvariablen}
    % (TODO: Schwingungen Zeichnen (oft cos s. VL 12.5.))
    ...
    \begin{center}
        \includegraphics[width=.8\linewidth]{img/no-dampened.png} \\
        \includegraphics[width=.8\linewidth]{img/weak-dampened.png} \\
        \includegraphics[width=.8\linewidth]{img/aperiodic-dampened.png} \\
        \includegraphics[width=.8\linewidth]{img/strong-dampened.png} \\
    \end{center}
    ...
    Sprungantwort \\$y_\sigma(t) = (d-\mathbf{c}^T\mathbf{A}^{-1}\mathbf{b})\sigma (t) + \mathbf{c}^T\mathbf{A}^{-1}\exp{(\mathbf{A}t)}\mathbf{b}\sigma(t)$ \\
        ...
        Für ein stabiles System mit $\lim_{t\rightarrow\infty}\exp{(\mathbf{A}t)} = \mathbf{0}$ ($\Re{\lambda_1}, \Re{\lambda_2} < 0$) konvergiert die Sprungantwort wie folgt: $y_{\sigma,\infty} = d + \mathbf{c}^T\mathbf{A}^{-1}\mathbf{b}$.
        \subsubsection{Impulsantwort}
    ...
    $\mathbf{y}(t) = \mathbf{y}_{\text{trans}}(t) + \mathbf{y}_{\text{steady}}(t)$
        \subsubsection{Steady-State-Antwort}
        Antwort im eingeschwungenen Zustand \\
    $\mathbf{y}_{\text{steady}}(t) = \text{Re}\{Y(j\omega)\e^{j\omega t}\}$\\
    $Y_{\text{steady}}(j\omega) = H(j\omega) \cdot U_{\text{in}}$ \\
        \subsubsection{Transientenantwort}
        Antwort des Systems während des Einschwingzustands. $\rightarrow 0$
        Die Summe der Polstellen von $H(j\omega)$ ist die Transientenantwort.\\
        $\mathbf{x}_{\text{trans}}(t)=\exp(\mathbf{A}t)\mathbf{x}(0)$ \\
    $\mathbf{y}_{\text{trans}}(t)=\sum_{i=1}^{n}c_i\vec v_ie^{\lambda_i t}$ \\

        \subsection{Federschwinger}
        Feder mit Federkonstante $k$ an der Decke befestigt mit einer Masse $m$ und Auslenkung $z$ nach unten.
        \begin{description}
        \item[Rückstellkraft] $F_F=kz$
        \item[Trägheit] $F_T=m\ddot z$
        \item[Gewichtskraft] $F_G=mg$
        \end{description}
        Kräftegleichgewicht: $F_F+F_T-F_G=0$, mit $\ddot z = \dot s$ folgt die DGL 2. Grades
        \begin{equation*}
            kz+m\dot s-mg = 0 \qquad \dot s = -\frac{kz}{m} + g
        \end{equation*}
        \begin{equation*}
            \begin{bmatrix}
                \dot z (t) \\ \dot s(t)
        ...
        Systeme beschrieben durch nichtlineare Funktionen $f$:
        \begin{align*}
            \dot x_1(t)=f_1(x_1(t), x_2(t)) \\
            \dot x_2(t)=f_2(x_1(t), x_2(t))
        \end{align*}

        \subsection{Zustandsbeschreibungen}
        \textbf{Kapazität}
        \begin{description}[labelindent=3mm]
        \item[spannungsgesteuert]: $u$ ist die Zustandsgröße
        \item[nur ladungsgesteuert]: $q$ ist die Zustandsgröße
        \end{description}

        \textbf{Induktivität}
        \begin{description}[labelindent=3mm]
        \item[stromgesteuert]: $i$ ist die Zustandsgröße
        \item[nur flussgesteuert]: $\Phi$ ist die Zustandsgröße
        \end{description}

        \begin{enumerate}
        \item Alle Gleichgewichtspunkte bestimmen $\mathbf{f}(x_\infty) \stackrel{!}{=} \mathbf{0}$
        \item Jacobimatrix bestimmen
        \item Gleichgewichtspunkte in Jacobimatrix einsetzen und Eigenwerte und Eigenvektoren bestimmen
        \item Überprüfen des \textbf{Satzes von Hartmann/Grobmann}: \\Für alle Eigenwerte muss gelten: $\Re{\lambda_i} \neq 0$, ansonsten kann man die Linearisierung mit Jacobimatrix nur bei konservativen Systemen anwenden
        \item Phasenportrait zeichnen (lokale Phasenportraits (Phasenportrait zu einem bestimmten GGP) stetig verbinden (Richtungen der Trajektorien bleiben alle gleich))
        \end{enumerate}
        \subsection{Energiefunktion $E(\mathbf{x})$}
        Eigenschaften: stetig, lokal nicht konstant, auf jeder Trajektorie konstant\\
        Schaltung ist \textbf{konservativ}, falls:\\
    $\dot{E} = 0 \Leftrightarrow \frac{\partial E(\mathbf{x})}{\partial x_1} \dot{x}_1(t) + \frac{\partial E(\mathbf{x})}{\partial x_2} \dot{x}_2(t) + \hdots + \frac{\partial E(\mathbf{x})}{\partial x_n} \dot{x}_n(t) = 0$ \\
    oder falls System verlustlos
        \subsubsection{Eigenschaften konservativer Schaltungen}
        \sectionbox{
        \begin{itemize}
        \item GGP ist entweder Sattelpunkt, Wirbelpunkt, oder GGP mit beiden $\lambda_i=0$ $\rightarrow$ keine rein stabilen GGPs
        \item Erweiterung des Satzes von Hartmann/Grobmann: Jacobimatrix hat nur imaginäre Eigenwerte und Schaltung ist konservativ $\Leftrightarrow$ GGP ist Wirbelpunkt
        \end{itemize}
        }
        \subsubsection{Zeichnen des Phasenportraits bei konservativen Schaltungen}
        \begin{enumerate}
        \item Energiefunktion gegeben durch z.B. $E(\xi_1,\xi_2)=...$
        \item Befindet man sich auf der Trajektorie $\Gamma$, so gilt $E(\Gamma)=E(\xi_1,\xi_2)=...$
        \item Löst man nach z.B. $\xi_2$ auf, so erhält man die Funktion $\xi_2(\xi_1)$, welche man zeichnen kann
        \end{enumerate}

        \subsection{Oszillatoren}
        Autonome Schaltung mit periodischem Verlauf der Zustandsgrößen, welcher unabhängig vom Anfangszustand ist\\
        ...
        \textbf{Relaxationsoszillator:} Oszillator mit $L \rightarrow 0$ und $F$ S-Kennlinie\\
        ...
        \sectionbox{
        \subsection{Lotka-Volterra-Gleichungen}
        Biologisches System mit Beute $p_1(t)$ und Jäger $p_2(t)$ konservativ.
        \begin{align*}
            \dot{p}_1(t)=[\alpha_1-\beta_1p_2(t)]p_1(t) \\
            \dot{p}_2(t)=[-\alpha_2+\beta_2p_1(t)]p_2(t)
        \end{align*}
        }

        \section{Dynamische Schaltungen beliebigen Grades}
        \subsection{Verallgemeinerte Zustandsgleichungen}
    $\frac{d}{dt} \underbrace{\begin{bmatrix}\mathbf{0} & \mathbf{0}\\\mathbf{0} & \mathbf{0}\\\mathbf{M}_1 & \mathbf{N}_1\end{bmatrix}}_{\ma D}\begin{bmatrix}\mathbf{u}\\\mathbf{i}\end{bmatrix} = \underbrace{ - \begin{bmatrix} \mathbf{B} & \mathbf{0} \\ \mathbf{0} & \mathbf{A} \\ \mathbf{M}_0 & \mathbf{N}_0\end{bmatrix}}_{\ma F}\begin{bmatrix}\mathbf{u}\\\mathbf{i}\end{bmatrix} + \begin{bmatrix}\mathbf{0}\\\mathbf{0}\\\mathbf{e}\end{bmatrix}$\\
    ...
    $\mathbf{B}$: KVL, $\mathbf{A}$: KCL
        ...
        \subsubsection{Grad der Schaltung}
        Der eigentliche Grad der Schaltung ist gegeben durch den Grad von $\det (\lambda \ma D- \ma F)=0$
        \subsubsection{Lösung TODO}

        \section{Analyse dynamischer Systeme im Frequenzbereich}
        \subsection{Laplace-Transformation}
        Für kausale Funktionen mit $f(t) = 0$ für $t < 0$:\\
    $F(s) = \mathcal L\{f(t)\} = \int_0^\infty f(t) \e^{-pt}dt$ mit $p$: komplexe Frequenz\\
        Rücktransformation: \\
    $f(t) = \mathcal L^{-1}\{F(s)\} = \fr{1}{2\pi j}\int_{\gamma-j\infty}^{\gamma+j\infty} F(s) \e^{-pt}dt$\\
        \subsubsection{Rechenregeln}
        \begin{itemize}
        \item Linearität: $\mathcal L\{\alpha f(t) + \beta g(t)\} = \alpha F(s) + \beta G(s)$
        \item Differentiationssatz: $\mathcal L\{\dot f(t)\} = pF(s) - f(0)$
        \item Faltung: $\mathcal L\{(a * b)(t)\} = A(s)B(s)$
        \end{itemize}
        \sectionbox{
        \subsubsection{Wichtige Laplace-Transformationen}
        \begin{itemize}
        \item $\mathcal L\{\alpha\e^{\beta t}\} = \fr{\alpha}{p-\beta}$
        \item $\mathcal L\{\e^{\ma At}\} = (p\ma 1 - \ma A)^{-1}$
        \item $\mathcal L\{\delta(t)\}=1$
        \end{itemize}
        \subsubsection{Allgemeine Lösung im Frequenzraum}
    $\vec X(s) = \underbrace{(p\ma 1-\ma A)^{-1}\vec x(0)}_{\vec X_{ZI}(s)} + \underbrace{(p\ma 1-\ma A)^{-1}\vec bV(s)}_{\vec X_{ZS}(s)}$\\
    $\vec x(t) = \vec x(0)\e^{\ma At} + \int_0^t \e^{\ma A(t-t')}v(t')\vec b dt'$
        }

        \subsection{Reduzierte Knotenspannungsanalyse}
        \begin{equation*}
            \underbrace{\ma Y_k}_{\text{Knotenadmittanzmatrix}}\vec U_k(s)=\vec I_q(s)
        \end{equation*}
        \textbf{Eintragen von Reaktanzen}
        \begin{enumerate}
        \item[] \begin{description}
        \item[Kapazität] $pC_n$
        \item[Induktivität] $\frac{1}{pL_n}$
        \end{description}
        \end{enumerate}
        ...
        \sectionbox{
        \emphbox{
    $H(j\omega ) = H(s) = \mathcal{L}\{h(t)\} = \fr{\text{Ausgang}}{\text{Eingang}}$
        } \\

        \begin{itemize}
        \item Beschreibt die Zero-State-Response
        \item $p_{0,i}$: Nullstellen des Ausgangs
        \item \textbf{Eigenfrequenzen} $p_{\infty, i}$: Nullstellen des Eingangs \\
    $\equalhat$ Polstellen von $H(j\omega)$ \\
    $\equalhat$ Charakteristisches Polynom von $\ma A$ \\
    $\equalhat$ Eigenwerte von $\ma A$
        \end{itemize}

        \vspace{.5em}
    $Y(s)=H(s)V(s) \rightarrow y(t)=h(t) * v(t)$ \\
    $y_{ZS}(t)=\mathcal L\{Y(s)\}$ \\
    $y_{ZI}(t)=\sum_{i=1}^{n}y_{0,i}e^{p_{\infty,i}}t$
        }

        \sectionbox{
        \subsubsection{3dB-Grenzfrequenz}
    $|H(j\omega)|^2 = \fr{1}{2} \Longleftrightarrow |\Re{H(j\omega_{\ir gr})}| = |\Im{H(j\omega_{\ir gr})}|$
        \begin{itemize}
        \item[] $v(\omega_{\ir gr})=20\log_{10}\frac{1}{\sqrt{2}}=-3dB$
        \end{itemize}
    ...
    $\varphi (\omega) = \angle H(j\omega) = \begin{cases}
    ...
    \end{cases}$\\
        ...
        \textbf{Serienschaltung von Übertragungsfunktionen} \\
    $H(s)=H_1(s)H_2(s) \rightarrow v(\omega) = v_1(\omega) + v_2(\omega), \\ \varphi(\omega) = \varphi_1(\omega) + \varphi_2(\omega)$ \\
        \textbf{Parallelschaltung von Übertragungsfunktionen} \\
    $H(s)=H_1(s) + H_2(s)$
    ...
    $H(jw) = K \cdot \prod_{n=1}^n \left(x_n+\frac{jw}{w_n}\right)^{z_n}$\qquad$z_n \in \mathbb{Z} \setminus \{0\}$\\
        ...
        \item Separates Zeichnen des Betrages $v(\omega)$ und der Phase $\varphi(\omega)$ in zwei Diagramme
        \end{enumerate}
        ...
        \item Umlaufrichtung von Punkten mit $\omega = 0$ zu Punkten mit $\omega \rightarrow \infty$
        \end{enumerate}
        \parbox{.6\linewidth}{
    ...
    $p_{\infty,1}p_{\infty,2}=\omega_0^2$ \\
        Bandbreite: $\frac{\omega_0}{Q}=|\omega_2 - \omega_1|$
        \section{SPICE}
        \renewcommand{\t}{\texttt}
        \subsection{Bauteile}
        \begin{itemize}
            \item \t{L<Name> <node1> <node2> <val>}: Spule zwischen Knoten \t{<node1>} und Knoten \t{<node2>} mit Induktivität \t{<val>}
            \item \t{C<Name> <node1> <node2> <val>}: Kondensator zwischen Knoten \t{<node1>} und Knoten \t{<node2>} mit Kapazität \t{<val>}
            \item \t{R<Name> <node1> <node2> <val>}: Widerstand zwischen Knoten \t{<node1>} und Knoten \t{<node2>} mit Widerstandswert \t{<val>}
            \item \t{U<Name> <node1> <node2> <val>}: Spannungsquelle zwischen Knoten \t{<node1>} und Knoten \t{<node2>} mit Spannung \t{<val>} (Spannung positiv bei \t{<node1>})
            \item \t{I<Name> <node1> <node2> <val>}: Stromquelle zwischen Knoten \t{<node1>} und Knoten \t{<node2>} mit Stromstärke \t{<val>} (Strom in Richtung \t{<node2>})
        \end{itemize}
        ...
        \end{itemize}
        \subsection{Quellenarten}
        \begin{itemize}
            \item \t{PULSE <Start> <End> <Delay> <tRise> <tFall> <tFall> <tOn>}
            \item \t{SINE <DcOffset> <Amplitude> <Freq> <tDelay> <Theta> <Phi>}
        \end{itemize}
        \subsection{Analysearten}
        \begin{itemize}
        \item \t{.tran <Endtime>}: Transientenanalyse im Zeitbereich von $t = 0$ bis $t = \text{\t{<Endtime>}}$. Bsp.: \t{.tran 60us}
    \end{itemize}
